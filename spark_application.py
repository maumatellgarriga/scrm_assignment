...
class ETL:
    ...
    def read_stores_v1(self, stores_v1_file_path: str) -> DataFrame:
        ...
        return (
            self.spark.read.schema(stores_schema)
            .option("header", True)
            .csv(stores_v1_file_path)
        )

    def read_stores_v2(self, stores_v2_file_path: str) -> DataFrame:
        ...
        return (
            self.spark.read.schema(stores_v2_schema)
            .option("header", True)
            .csv(stores_v2_file_path)
        )

    def read_ticket_lines(self, ticket_line_file_path: str) -> DataFrame:
        ...
        return (
            self.spark.read.schema(ticket_lines_schema)
            .option("header", True)
            .csv(ticket_line_file_path)
        )

    def read_products(self, products_file_path: str) -> DataFrame:
    ...
    def lp_stores(self, stores: DataFrame) -> List[int]:
        stores_integrated = stores.withColumn(
            "store_id",
            F.when(
        ...
        return [
            int(x["store_id"]) for x in (stores_integrated.select("store_id").collect())
        ]

    def total_stores_per_product(
        self, ticket_lines: DataFrame, stores: DataFrame
    ) -> DataFrame:
        return (
            ticket_lines.filter(F.col("store_id").isin(self.lp_stores(stores)))
            .groupBy("product_id")
            .agg(F.countDistinct("store_id").cast(T.IntegerType()).alias("n_stores"))
            .orderBy("product_id")
        )

    def second_most_selling_store_per_product(
        self, ticket_lines: DataFrame, stores: DataFrame
    ) -> DataFrame:
        sorted_quantities = Window.partitionBy("product_id").orderBy(
            F.col("quantity").desc()
        )
        return (
            ticket_lines.filter(F.col("store_id").isin(self.lp_stores(stores)))
            ...
            .withColumn(
                "second_quantity",
                F.collect_list(F.col("quantity")).over(sorted_quantities)[1],
            )
            .filter(F.col("second_quantity") == F.col("quantity"))
            .select(F.col("product_id"), F.col("store_id").alias("second_store_id"))
        )

    def second_popular_stores_by_product_category(
        self, ticket_lines: DataFrame, products: DataFrame, stores: DataFrame
    ) -> DataFrame:
        return (
            self.second_most_selling_store_per_product(ticket_lines, stores)
    ...
    def main(self, stores_version: int):
        ...
        second_store_per_product = self.second_most_selling_store_per_product(
            ticket_lines, stores
        )
        second_store_by_category = self.second_popular_stores_by_product_category(
            ticket_lines, products, stores
        )
